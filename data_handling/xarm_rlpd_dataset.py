...
def composite_fn(img, mask, rng, bg_images, bg_aug_threshold):
    ...
    def do_composite(_):
        # Pick a random background index
...
def crop_fn(img):
    ...
    # Create an all-black image of the same shape as img.
    black = jnp.zeros_like(img)
    # Copy the cropped portion from img into black.
...
class XarmRLDataset(Dataset):
        ...
        **kwargs,
        ...
        if bg_augs:
            ...
            for f in self.bg_files:
                # Read the image, convert to float32, and scale to [0,1]
            ...
            base_dir = Path(
                "/mnt/robotlab/zifan/bcrl_pipeline/0423_new_usb_inserion_green_screen/base/"
            )
    ...
    # LATERTODO: Change inheritance structure to use an XArmDataset and just Dataset
    ...
    def insert_from_timestep(self, time_step, bg_augs, preprocess: bool = False):
        # ipdb.set_trace()
        if bg_augs:
            frame = cv2.resize(
            ...
            if self.prev_mask is None:
                ...
                # 3) map each mask‐space point → image‐space via cosine similarity
                scale = 768 // 48  # your IMG_SIZE//FEATURE_MAP_SIZE
                mapped = []
                for mx, my in original_pts:
                    ...
                    if "card" in self.base_dir:
                        if yy > 20:
                            mapped.append([xx * scale, yy * scale])
                    elif "usb" in self.base_dir:
                        if 24 < yy < 36:
                            mapped.append([xx * scale, yy * scale])
                    else:
                        mapped.append([xx * scale, yy * scale])

                # 4) call SAM with all 5 points at once
                pts = np.array(mapped)
            ...
            # Call the track_mask method (this is the same as in your RobotEnv)
            self.mask_count += 1
        ...
        if not time_step.last():
            for k in ["reward", "action", "action_base", "discount"]:
                self.dataset_dict[k][self._insert_index] = time_step[k]
            ...
            if k.startswith("obs"):
                for obs_k in self.dataset_dict[k]:
                    ...
                    if obs_k.startswith("pixels"):
                        ...
                        if bg_augs:
                            # num_bg_files = len(self.bg_files)
                            ...
                            # Select the backgrounds from the preloaded array
