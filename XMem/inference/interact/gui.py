...
class App(QWidget):
    ...
    def load_current_torch_image_mask(self, no_mask=False):
        if self.current_image_torch is None:
            self.current_image_torch, self.current_image_torch_no_norm = image_to_torch(
                self.current_image, self.device
            )
    ...
    def update_minimap(self):
        ...
        bytesPerLine = 3 * width
        qImg = QImage(
            patch.data, width, height, bytesPerLine, QImage.Format.Format_RGB888
        )
    ...
    def update_current_image_fast(self):
        ...
        qImg = QImage(
            self.viz.data, width, height, bytesPerLine, QImage.Format.Format_RGB888
        )
        self.main_canvas.setPixmap(
            QPixmap(
                qImg.scaled(
    ...
    def on_export_visualization(self):
        ...
        if os.path.exists(image_folder):
            ...
            self.console_push_text(
                f'Exporting visualization to {self.config["workspace"]}/visualization.mp4'
            )
            images = [
                img for img in sorted(os.listdir(image_folder)) if img.endswith(".jpg")
            ]
            frame = cv2.imread(os.path.join(image_folder, images[0]))
            height, width, layers = frame.shape
            # 10 is the FPS -- change if needed
            video = cv2.VideoWriter(
                f"{save_folder}/visualization.mp4",
            ...
            video.release()
            self.console_push_text(
                f'Visualization exported to {self.config["workspace"]}/visualization.mp4'
            )
        else:
    ...
    def on_mouse_press(self, event):
        ...
        if event.button() == Qt.MouseButton.MiddleButton:
            ...
            self.console_push_text(
                f"Target objects for visualization changed to {self.vis_target_objects}"
            )
            self.show_current_frame()
            return
        ...
        if self.curr_interaction == "Scribble":
            if (
                ...
                new_interaction = ScribbleInteraction(
                    image,
                    torch.from_numpy(self.current_mask).float().to(self.device),
                    (h, w),
                    self.s2m_controller,
                    self.num_objects,
                )
    ...
    def on_import_mask(self):
        ...
        mask = self.res_man.read_external_image(
        ...
        if not shape_condition:
            self.console_push_text(
                f"Expected ({self.height}, {self.width}). Got {mask.shape} instead."
            )
        ...
        try:
            ...
            if layer.shape[-1] == 3:
                layer = np.concatenate(
                    [layer, np.ones_like(layer[:, :, 0:1]) * 255], axis=-1
                )
