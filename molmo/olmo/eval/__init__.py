...
def build_evaluator(
    ...
    eval_metric = dict(Loss=make_metric(), Accuracy=make_metric(), ZLoss=make_metric())
    return LossDatasetEvaluator(
        label=eval_config.label,
        eval_loader=eval_loader,
        eval_metric=eval_metric,
        subset_num_batches=eval_config.subset_num_batches
        or train_config.eval_subset_num_batches,
    )


def build_loss_evaluators(
    cfg: TrainConfig, device: torch.device
) -> List[LossDatasetEvaluator]:
    evaluators = []
...
def build_inf_evaluators(
    cfg: TrainConfig, device: torch.device
) -> List[InfDatasetEvaluator]:
    evaluators = []
    tokenizer = cfg.model.get_tokenizer()
    for eval_config in cfg.inf_evaluators:
        ...
        device_batch_size = (
            eval_config.device_eval_batch_size or cfg.device_inf_eval_batch_size
        )
        ...
        eval_loader = build_eval_dataloader(
            cfg, eval_config.data, device_batch_size, max_steps=max_steps
        )
