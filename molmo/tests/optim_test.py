...
def test_bolt_on_warmup_scheduler():
    ...
    assert scheduler2.get_lr(initial_lr, 5500, max_steps) == pytest.approx(
        0.25 * (1 + alpha_f)
    )
    assert scheduler2.get_lr(initial_lr, 6000, max_steps) == pytest.approx(
        0.5 * (1 + alpha_f)
    )
    assert scheduler2.get_lr(initial_lr, 7000, max_steps) == scheduler.get_lr(
        initial_lr, 7000, max_steps
    )
