...
class HPatchDataset(Dataset):
    def __init__(self, imdir, spdir):
        ...
        for f in os.listdir(imdir):
            ...
            self.imfs.extend(
                [os.path.join(scene_dir, "{}.ppm").format(ind) for ind in range(1, 7)]
            )
        self.spdir = spdir
    ...
    def __getitem__(self, item):
        ...
        coord = np.loadtxt(os.path.join(self.spdir, f"{name}-{idx[0]}.kp")).astype(
            np.float32
        )
...
def main(args):
    ...
    with torch.no_grad():
        for data in tqdm(data_loader):
            ...
            c = torch.Tensor([(w - 1) / 2.0, (h - 1) / 2.0]).to(coord.device).float()
            ...
            out_dir = os.path.join(
                args.save_path, os.path.basename(os.path.dirname(img_path))
            )
            os.makedirs(out_dir, exist_ok=True)
            with open(
                os.path.join(
                    out_dir, f"{os.path.basename(img_path)}.dift_{args.dift_model}"
                ),
                "wb",
            ) as output_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--kpts_path",
        type=str,
        default="./superpoint-1k",
        help="path to 1k superpoint keypoints",
    )
    parser.add_argument(
        "--save_path",
        type=str,
    ...
    parser.add_argument(
        ...
        choices=["sd", "adm"],
        default="sd",
        help="which dift version to use",
    )
    parser.add_argument(
        "--img_size",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="which upsampling block to extract the ft map",
    )
    parser.add_argument(
        "--ensemble_size",
        default=8,
        type=int,
        help="ensemble size for getting an image ft map",
    )
