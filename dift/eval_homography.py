...
def mnn_matcher(descriptors_a, descriptors_b, metric="cosine"):
    ...
    if metric == "cosine":
        ...
        dist = (
            torch.sum(descriptors_a**2, dim=1, keepdim=True)
            + torch.sum(descriptors_b**2, dim=1, keepdim=True).t()
...
def generate_read_function(save_path, method, extension="ppm", top_k=None):
    def read_function(seq_name, im_idx):
        aux = np.load(
            os.path.join(save_path, seq_name, "%d.%s.%s" % (im_idx, extension, method))
        )
        if top_k is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hpatches_path",
        type=str,
        default="/scratch/dift_release/d2-net/hpatches_sequences/hpatches-sequences-release",
        help="path to hpatches dataset",
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default="./hpatches_results",
        help="path to save features",
    )
    parser.add_argument(
        "--feat",
        choices=["dift_sd", "dift_adm"],
        default="dift_sd",
    ...
    parser.add_argument(
        ...
        default="lmeds",
        help="which method to use when calculating homography",
    )
    ...
    for seq_idx, seq_name in tqdm(enumerate(seq_names)):
        ...
        for im_idx in range(2, 7):
            h2, w2 = cv2.imread(
                os.path.join(args.hpatches_path, seq_name, "{}.ppm".format(im_idx))
            ).shape[:2]
            ...
            H_gt = np.loadtxt(
                os.path.join(args.hpatches_path, seq_name, "H_1_" + str(im_idx))
            )
            ...
            if args.mode == "ransac":
                H, mask = cv2.findHomography(
                    pts_a, pts_b, cv2.RANSAC, ransacReprojThreshold=3
                )
            elif args.mode == "lmeds":
                H, mask = cv2.findHomography(
                    pts_a, pts_b, cv2.LMEDS, ransacReprojThreshold=3
                )
    ...
    overall_mean_accuracy = np.mean(
        np.concatenate((i_accuracy, v_accuracy), axis=0), axis=0
    )
    print(
        "overall_acc: {}, i_acc: {}, v_acc: {}".format(
