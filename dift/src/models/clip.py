...
def interpolate_pos_encoding(
    clip_model, embeddings: torch.Tensor, height: int, width: int
) -> torch.Tensor:
    """
    ...
    def forward(
        ...
        if clip_model.input_patchnorm:
            ...
            x = x.reshape(
                x.shape[0],
                x.shape[1],
                clip_model.grid_size[0],
                clip_model.patch_size[0],
                clip_model.grid_size[1],
                clip_model.patch_size[1],
            )
            x = x.permute(0, 2, 4, 1, 3, 5)
            x = x.reshape(
                x.shape[0], clip_model.grid_size[0] * clip_model.grid_size[1], -1
            )
        ...
        x = torch.cat(
            [
                clip_model.class_embedding.to(x.dtype)
                + torch.zeros(
        ...
        for i, r in enumerate(clip_model.transformer.resblocks):
            ...
            if i == block_index:
                ...
                tokens = (
                    ...
                    .contiguous()
                    .view(batch_size, num_channel, ft_size, ft_size)
                )  # NCHW
