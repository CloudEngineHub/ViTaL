...
class ADMFeaturizer:
    def __init__(self):
        ...
        model_path = os.path.join(
            main_path, "guided-diffusion/models/256x256_diffusion_uncond.pt"
        )
    ...
    @torch.no_grad()
    def forward(self, img_tensor, t=101, up_ft_index=4, ensemble_size=8):
        model = self.model
...
class ADMFeaturizer4Eval(ADMFeaturizer):
    @torch.no_grad()
    def forward(
        self, img, img_size=[512, 512], t=101, up_ft_index=4, ensemble_size=8, **kwargs
    ):
        img_tensor = self.adm_transforms(img.resize(img_size))
        ft = super().forward(
            img_tensor, t=t, up_ft_index=up_ft_index, ensemble_size=ensemble_size
        )
        return ft
